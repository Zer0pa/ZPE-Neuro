<p>
  <img src="../../../.github/assets/readme/zpe-masthead.gif" alt="ZPE-Neuro Masthead" width="100%">
</p>

<p>
  <img src="../../../.github/assets/readme/section-bars/proof-surface.svg" alt="PROOF SURFACE" width="100%">
</p>

# Release Alignment Packet

This packet is the tracked March 21, 2026 release-surface verification summary
for ZPE-Neuro.

## What It Contains

- [gate_c_summary.json](gate_c_summary.json)
- [gate_d_summary.json](gate_d_summary.json)
- [falsification_results.md](falsification_results.md)
- [gate_c_command_log.txt](gate_c_command_log.txt)
- [gate_d_command_log.txt](gate_d_command_log.txt)
- [verification_summary.md](verification_summary.md)

## What It Does Not Contain

- wheel or sdist binaries
- ignored local `artifacts/` directories
- blind-clone authority-pack evidence

Those remain outside the shipped proof packet or remain open.

This packet supports the current useful-now repo surface for bounded
extracellular replay and falsification truth. Blind-clone authority replay
remains a separate gate.
